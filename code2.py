...
class RMMModel:
    ...
    def reevaluate_memory(self, memory):
        ...
        if "Greedy" in self.value_system_principles:
            # Check for gain-seeking, recklessness
            if "gain" in memory.current_inference.lower() or "risky" in memory.current_inference.lower():
                potential_issues.append("Inference might reflect excessive 'Greedy' value clash or recklessness.")
                needs_re_evaluation_trigger = True
        if "Angry" in self.value_system_principles:
            # Check for anger-related terms or inferences that justify anger
            if "upset" in memory.current_inference.lower() or "angry" in memory.current_inference.lower() or "rage" in memory.current_inference.lower():
        ...
        if "Affiliational" in self.value_system_principles:
            # Check for terms related to isolation or lack of social connection/concern
            if "alone" in memory.current_inference.lower() or "isolated" in memory.current_inference.lower() or "self-centered" in memory.current_inference.lower():
                potential_issues.append("Inference might clash with 'Affiliational' value (e.g., promoting isolation or lack of empathy).")
                needs_re_evaluation_trigger = True
        ...
        if potential_issues and random.random() < 0.7: # A higher chance of re-evaluation if issues flagged
            ...
            new_inference_candidate = "updated inference based on value system review of: " + memory.content
...
if __name__ == "__main__":
    ...
    # --- Initial Memories (from previous example) ---
    ...
    # --- Bike Fall Test Cases ---
    print("\n" + "="*50)
    print("Beginning Bike Fall Specific Test Cases")
    print("="*50)

    # Scenario: bike fell, man driving in snake style, sober = Not Strange
    print("\n--- Test Case: Bike fell, snake style, sober (Expected: Not Strange/No major clash) ---")
    # This might not trigger a strong value clash, but a "risky" inference could still be flagged
    rmm_system.add_memory("bike_snake_sober", "Bike fell, man driving in snake style, sober.", "Riding in snake style is just a peculiar habit.", initial_valence=-0.2)
    rmm_system.run_rmm_cycle("bike_snake_sober")
    print(f"Final State: {rmm_system.memories['bike_snake_sober']}")

    # Scenario: bike fell, man driving in snake style, drunk = Possible (Angry/Greedy implications)
    print("\n--- Test Case: Bike fell, snake style, drunk (Expected: Possible clash, especially Angry/Greedy) ---")
    rmm_system.add_memory("bike_snake_drunk", "Bike fell, man driving in snake style, drunk.", "Drunk driving is dangerous and irresponsible for self and others.", initial_valence=-0.9)
    rmm_system.run_rmm_cycle("bike_snake_drunk")
    print(f"Final State: {rmm_system.memories['bike_snake_drunk']}")

    # Scenario: bike fell when man driving free hand, sober = Not Strange
    print("\n--- Test Case: Bike fell, free hand, sober (Expected: Not Strange/No major clash) ---")
    # Again, might not trigger a strong clash, could be seen as a skill attempt
    rmm_system.add_memory("bike_freehand_sober", "Bike fell, man driving free hand, sober.", "Freehand riding is a stunt, not inherently problematic.", initial_valence=-0.3)
    rmm_system.run_rmm_cycle("bike_freehand_sober")
    print(f"Final State: {rmm_system.memories['bike_freehand_sober']}")


    # Scenario: bike fell when man driving free hand, drunk = Possible (Angry/Greedy implications)
    print("\n--- Test Case: Bike fell, free hand, drunk (Expected: Possible clash, especially Angry/Greedy) ---")
    rmm_system.add_memory("bike_freehand_drunk", "Bike fell, man driving free hand, drunk.", "Drunk and reckless driving is selfish and risky for all.", initial_valence=-0.95)
    rmm_system.run_rmm_cycle("bike_freehand_drunk")
    print(f"Final State: {rmm_system.memories['bike_freehand_drunk']}")

    # Scenario: bike fell when man driving angerily, sober = Possible (Angry implications)
    print("\n--- Test Case: Bike fell, angrily, sober (Expected: Possible clash, especially Angry) ---")
    rmm_system.add_memory("bike_angry_sober", "Bike fell, man driving angrily, sober.", "His anger made him reckless and caused the fall.", initial_valence=-0.7)
    rmm_system.run_rmm_cycle("bike_angry_sober")
    print(f"Final State: {rmm_system.memories['bike_angry_sober']}")

    # Scenario: bike fell when man driving angerily, drunk = Highly likely (Angry/Greedy implications)
    print("\n--- Test Case: Bike fell, angrily, drunk (Expected: Highly likely clash, Angry/Greedy/Affiliational) ---")
    rmm_system.add_memory("bike_angry_drunk", "Bike fell, man driving angrily and drunk.", "This was a result of uncontrolled anger and complete disregard for safety.", initial_valence=-1.0)
    rmm_system.run_rmm_cycle("bike_angry_drunk")
    print(f"Final State: {rmm_system.memories['bike_angry_drunk']}")


    print("\n" + "="*50)
    print("End of Bike Fall Specific Test Cases")
    print("="*50)

    # --- Original Final Memory States Check (retained for completeness) ---
    print("\n--- Final States of ALL Memories After All Cycles ---")
